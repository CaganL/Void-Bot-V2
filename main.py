...
# --- GEMINI: SENARYO VE SAHNE PLANLAMA ---
def get_content(topic):
    # En iyi modellerden en hızlısına doğru dene
    models = [
    ...
    base_prompt = (
        ...
        "CRITICAL RULES (V112 PROFESSIONAL MODE):\n"
        "1. **NO STORYTELLING:** No 'ran away', no 'screamed', no 'help me'.\n"
        "2. **CATASTROPHIC ENDING:** System failure (e.g., 'Spine severed', 'Skull fractured').\n"
        "3. **VISUAL METAPHORS:** Suggest 'Cracking ice', 'Snapping branch', 'Red ink in water' for impact moments.\n"
        "4. **STYLE:** Cold, Clinical. Subject + Verb + Object.\n"
        "5. **LENGTH:** 45-55 Words (Optimized for ElevenLabs pacing)."
    )
            ...
            if r.status_code == 200:
                ...
                if 'candidates' in response_json and response_json['candidates']:
                    ...
                    if len(parts) >= 8:
                        ...
                        if word_count > 65 or word_count < 35: continue # Tolerans aralığı
                        
                        print(f"✅ ZAFER! {current_model} başardı. Kelime: {word_count}")

                        # Görsel Arama Kelimeleri Oluştur
                        ...
                        modifiers = ["medical", "x-ray", "mri", "microscope", "cinematic lighting"]
                        for variant in search_variants:
                            for mod in modifiers:
                                visual_queries.append(f"{variant} {mod}")
                        
                        # Soyut/Metaforik Aramalar
                        ...
                        return {
                            ...
                            "tags": " ".join([t for t in parts[7].strip().replace(",", " ").split() if t.startswith("#")]),
...
def search_mixkit(query):
    ...
    try:
        ...
        return random.choice(videos[:5]).get('src') # İlk 5'ten rastgele seç
    except: pass
    return None
...
def search_pixabay(query):
    ...
    try:
        url = f"https://pixabay.com/api/videos/?key={PIXABAY_API_KEY}&q={query}&video_type=film&per_page=10" 
...
def generate_elevenlabs_audio(text, filename):
    ...
    # Eleven Turbo v2.5 (En hızlı ve ucuz model)
    data = {
        "text": text,
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"✅ ElevenLabs Sesi Oluşturuldu: {filename}")
            return True
        else:
            print(f"❌ ElevenLabs Hatası ({response.status_code}): {response.text}")
            return False
    except Exception as e:
        print(f"❌ ElevenLabs Bağlantı Sorunu: {e}")
        return False

# --- KAYNAK OLUŞTURMA VE MONTAJ HAZIRLIĞI ---
async def generate_resources(content):
    ...
    if not generate_elevenlabs_audio(hook, "hook.mp3"):
        print("⚠️ Hook için Edge-TTS kullanılıyor.")
...
def clinical_grade(image):
    # Soğuk, Yeşil/Mavi 'Adli Tıp' Filtresi
    img_f = image.astype(float)
    gray = np.mean(img_f, axis=2, keepdims=True)
    desaturated = img_f * 0.4 + gray * 0.6 
    tint_matrix = np.array([0.8, 1.1, 1.2]) # Yeşil-Mavi baskın
    graded_img = desaturated * tint_matrix
    graded_img = (graded_img - 128) * 1.2 + 128 # Kontrast artır
    ...
    try:
        ...
        for i, p in enumerate(paths):
            try:
                ...
                is_impact = False
                # Videonun son saniyeleri ise X-Ray yap
                if i >= len(paths) - 1: is_impact = True
...
def handle(message):
    try:
        ...
        msg = bot.reply_to(message, f"💀 **{topic.upper()}**\nProfesyonel Mod (V112)...\n")
        
        content = get_content(topic)
        
        if not content:
            bot.edit_message_text("❌ Hikaye kurallara uymadı veya API hatası.", message.chat.id, msg.message_id)
            return
